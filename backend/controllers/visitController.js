...
exports.createVisit = async (req, res) => {
  ...
  if (!vId) {
    const visitorRow = await pool.query(
      "SELECT visitor_id FROM visitors WHERE phone=$1",
      [visitor_phone]
    );
    if (visitorRow.rows.length) {
      ...
      const newVisitor = await pool.query(
        "INSERT INTO visitors (name,phone) VALUES ($1,$2) RETURNING visitor_id",
  ...
  const result = await pool.query(
    `INSERT INTO visits (visitor_id, flat_id, guard_id, visitor_type, purpose, company_name)
     VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
    [vId, flat_id, guard_id, visitor_type, purpose, company_name]
  );
